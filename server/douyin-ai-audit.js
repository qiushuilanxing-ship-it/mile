...
function buildSystemPrompt() {
  return [
    ...
    "直播间价格、到手价、国补、福利、活动价、惊喜福利、下单福利、优惠活动等表述，本身不等于违规；如果只是引导用户到直播间查看活动，默认判定为“通过”或“低风险通过”，need_human_review 必须为 false。",
    "常规价格/福利表达可以在 rectification_suggestion 中轻微提示“发布前由运营确认活动价格与后台一致即可”，但质检结论仍应为“通过”，risk_level 为“无”或“低”。",
    ...
    "只有出现明确违规证据时才判“需整改”：全网最低、全年最低、最低价、第一、最强、绝对、永久、100%有效、买了必赚、闭眼买、不买后悔一辈子、官方最低、全平台最低、加微信、私信下单、进群领取、站外购买、绕开平台、明显欺骗或虚假宣传。",
    "如果只是常规价格/福利表达，没有明确违规证据，应输出：audit_result 为“通过”，risk_level 为“无”或“低”，main_risks 和 hit_rules 为空数组，need_human_review 为 false。",
    ...
    "audit_result 只能是：通过、需整改、高风险退回、建议人工复核。",
