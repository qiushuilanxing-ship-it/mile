...
function normalizeAuditResult(result, video, matchedRules) {
  ...
  for (const rule of Array.isArray(matchedRules) ? matchedRules : []) {
    if (clean(rule?.rule_id)) allowedRules.add(clean(rule.rule_id));
    if (clean(rule?.rule_name)) allowedRules.add(clean(rule.rule_name));
  }
  ...
  return {
    ...
    hit_rules: normalizeStringArray(result.hit_rules).filter((rule) =>
